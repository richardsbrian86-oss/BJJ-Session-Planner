...
export default function InstructorForgotPin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/instructors/forgot-pin", {
        method: "POST",
          ...
          {sent ? (
            <div style={{ textAlign: "center" }}>
              ...
              <p style={{ color: "#8b9ab3", fontSize: 14, margin: "0 0 24px", lineHeight: 1.6 }}>
                If an instructor account exists for <strong style={{ color: "#eef0f4" }}>{email}</strong>, we've sent a PIN reset link. It expires in 1 hour.
              </p>
              <Link href="/instructor/login" style={{ color: ACCENT, fontWeight: 600, textDecoration: "none", fontSize: 14 }}>Back to sign in</Link>
            </div>
          ) : (
            <>
              <h1 style={{ fontSize: 24, fontWeight: 800, color: "#eef0f4", margin: "0 0 4px", letterSpacing: "-0.02em" }}>Reset your PIN</h1>
              <p style={{ color: "#4a5568", fontSize: 14, margin: "0 0 24px" }}>Enter your instructor email and we'll send a PIN reset link</p>
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <button type="submit" disabled={loading} style={{ width: "100%", background: loading ? "rgba(26,171,144,0.5)" : `linear-gradient(135deg, ${ACCENT} 0%, ${ACCENT_DARK} 100%)`, color: "#fff", fontWeight: 700, fontSize: 15, padding: "14px", border: "none", borderRadius: 11, cursor: loading ? "not-allowed" : "pointer", boxShadow: loading ? "none" : `0 4px 16px rgba(26,171,144,0.35)`, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontFamily: "inherit", marginTop: 4 }}>
                  {loading ? <><Loader2 style={{ width: 16, height: 16, animation: "spin 1s linear infinite" }} />Sending...</> : "Send PIN Reset Link"}
                </button>
              </form>
              ...
              <div style={{ marginTop: 14, textAlign: "center" }}>
                <p style={{ fontSize: 12, color: "#4a5568", margin: 0, lineHeight: 1.6 }}>
                  No email on file? Open the Let's Roll mobile app and use <strong style={{ color: "#8b9ab3" }}>Reset Account</strong> to re-register.
                </p>
              </div>
            </>
          )}
